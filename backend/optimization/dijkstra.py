...
def run_dijkstra(
    ...
    refs: Dict[str, float],
    routing_mode: str = "multi_objective",
) -> dict:
    """
    Find the minimum-cost path from src to dst using Dijkstra's algorithm.

    Parameters:
        routing_mode: "multi_objective" (default, minimizes balanced fitness)
                      "classical" (minimizes standard road distance, representing
                      classical single-objective GPS routing for SIH benchmarking).
    """
    ...
    def weight_fn(u: int, v: int, edge_dict: dict) -> float:
        ...
        for k, data in edge_dict.items():
            ...
            if overlay.get("closed", False):
                continue
            if routing_mode == "classical":
                c = data.get("length", 0.0)
            else:
